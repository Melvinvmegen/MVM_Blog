...
1. First we sort our 2D array by array length with [sort()](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/sort) to make sure that the longest array is always first so that the order at which we provide them doesn't matter.
...
::note
**Note:** We could extend this function to match objects as well with any comparison you'd like, for the sake of the example let's compare all the properties of the object by first converting it to a string like this:
