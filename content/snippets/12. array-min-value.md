...
- Second, inside the [reduce()](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/Reduce) method, we compare the **current value** with the **previous smallest value** (starting with an empty object). If the **current value** has a lower value than the **previous smallest value**, it is replaced by the **current value**.
