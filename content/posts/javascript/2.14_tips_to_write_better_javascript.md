...
I consider it a **code smell** to define too many parameters in your function. Even if parameters have default value or are optional, let's take this example:
...
Both **members** and **admins** assignments only differ by the [logical not (!)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Logical_NOT) but it's not  and if you need to change one assignment, then you also need to change the other one.
